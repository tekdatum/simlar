...
Adapted from test_bm25x_index.py (the other external-BM25-engine integration, same pattern)
rather than test_relevance_index.py, since both are "hardcode an external engine into simlar"
wrappers. Two things deliberately differ from that template, confirmed empirically rather than
assumed:
  - bm25c/BM25CRelevanceCore mirrors _RelevanceCore's own behavior, not bm25x's: duplicate ids
...
class TestMutations:
    ...
    def test_add_duplicate_id_silently_allowed(self):
        # Matches _RelevanceCore's own documented behavior (not bm25x's, which rejects this) -
