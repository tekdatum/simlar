#!/usr/bin/env python
"""bm25c vs RelevanceIndex (bm25s) -- build/search timing comparison.

Deliberately has none of examples/10_rags_benchmark.py's RAG/framework layer
(no LangChain/LlamaIndex/Haystack, no embeddings, no LLM) -- both are
...
query on its own OpenMP thread, and _RelevanceCore's search_raw does the
equivalent via bm25s' numba backend), and retrieval-quality (Hit@1/Hit@k/MRR) using the same
...
find where bm25c loses ground to bm25s, and each engine's own
...
from simlar import BM25CIndex, RelevanceIndex
...
def benchmark(name: str, index, dataset: SquadDataset, k: int, build_ms: float) -> dict:
    ...
    # cost it actually is. bm25c has no such warm-up (already-compiled C),
    # so this is a no-op cost for it either way.
    ...
    # sentinel bm25c uses for "fewer than k real matches";
